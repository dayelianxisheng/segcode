...
def parse_args():
    p = argparse.ArgumentParser(description="DANet Training")
    ...
    p.add_argument("--weight_path", type=str, default="params/danet_ship.pth")
...
def create_model(args):
    if args.model == "danet":
        return DANet(num_classes=1, backbone=args.backbone, pretrained=args.pretrained), True
    elif args.model == "danet_simple":
        return DANetSimple(num_classes=1, backbone=args.backbone, pretrained=args.pretrained), False


def train_one_epoch(model, loader, criterion, optimizer, device, interval, use_multi_output):
    model.train()
    total = 0
...
def validate(model, loader, criterion, device, metrics, use_multi_output):
    model.eval()
    total = 0
    with torch.no_grad():
        for x, y in loader:
...
def main():
    ...
    model, use_multi_output = create_model(args)
    model = model.to(device)
    model_name = f"DANet-{args.backbone}" if args.model == "danet" else f"DANetSimple-{args.backbone}"
    ...
    logger.dataset_info(len(train_ds), len(val_ds), args.batch_size,
                        args.val_batch_size, image_size, args.num_workers)
