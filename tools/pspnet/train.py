...
from model import DiceLoss, CombinedLoss, PSPNetLoss
...
def parse_args():
    p = argparse.ArgumentParser(description="PSPNet Training")
    ...
    p.add_argument("--use_ppm", action="store_true", default=True)
    ...
    p.add_argument("--weight_path", type=str, default="params/pspnet_ship.pth")
...
def create_model(args):
    if args.model == "pspnet":
        ...
        return PSPNet(num_classes=1, backbone=args.backbone, pretrained=args.pretrained, use_ppm=args.use_ppm), False
    elif args.model == "pspnet_aux":
        from model import PSPNetWithAux
        return PSPNetWithAux(num_classes=1, backbone=args.backbone, pretrained=args.pretrained), True


def train_one_epoch(model, loader, criterion, optimizer, device, interval, use_aux):
    model.train()
    total = 0
    for i, (x, y) in enumerate(loader):
        ...
        if use_aux:
            out, aux_out = model(x)
            loss, loss_dict = criterion(out, y, aux_out)
        else:
            out = model(x)
...
def validate(model, loader, criterion, device, metrics, use_aux):
    model.eval()
    total = 0
    with torch.no_grad():
        for x, y in loader:
            ...
            if use_aux:
                out, aux_out = model(x)
                loss, _ = criterion(out, y, aux_out)
            else:
                out = model(x)
                loss = criterion(out, y)
            total += loss.item()
...
def main():
    ...
    model, use_aux = create_model(args)
    model = model.to(device)
    model_name = f"PSPNet-{args.backbone}" if args.model == "pspnet" else f"PSPNetWithAux-{args.backbone}"
    params = count_parameters(model)
    logger.model_info(model_name, params, device)
    
    if use_aux:
        criterion = PSPNetLoss(aux_weight=args.aux_weight, use_combined=True, bce_weight=args.bce_weight, dice_weight=args.dice_weight)
    else:
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        train_loss = train_one_epoch(model, train_loader, criterion, optimizer, device, args.print_interval, use_aux)
        print(f"Train Loss: {train_loss:.4f}")
        
        metrics_tracker = SegmentationMetrics()
        val_loss, metrics = validate(model, val_loader, criterion, device, metrics_tracker, use_aux)
