...
from model import ResUnetPlusPlus, create_res_unet_plus
...
def parse_args():
    """解析命令行参数"""
    p = argparse.ArgumentParser(description="ResUNet++ Training")
    p.add_argument("--data_path", type=str, default=r"D:\resource\data\seg\AirbusShip_filtered_0.5",
                   help="数据集路径")
    p.add_argument("--weight_path", type=str, default="params/resunet_plus_ship.pth",
                   help="模型权重保存路径")
...
def main():
    ...
    # 创建模型
    print(f"创建ResUNet++模型")
    model = create_res_unet_plus(
        in_channels=3,
        num_classes=1
    ).to(device)

    params = count_parameters(model)
    logger.model_info("ResUNet++", params, device)
