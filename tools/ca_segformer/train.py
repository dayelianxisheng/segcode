...
from model import CASegFormer, CombinedLoss
...
def parse_args():
    p = argparse.ArgumentParser(description="SegFormer Training")
    p.add_argument("--phi", type=str, default="b0", choices=["b0", "b1", "b2", "b3", "b4", "b5"])
    ...
    p.add_argument("--weight_path", type=str, default="params/segformer_ship.pth")
...
def main():
    ...
    model = CASegFormer(num_classes=1, phi=args.phi, pretrained=args.pretrained).to(device)
    model_name = f"CASegFormer-{args.phi}"
    ...
    train_dataset = AirbusDataset(args.data_path, split="training", image_size=image_size,
                                   use_augmentation=args.use_augmentation)
    val_dataset = AirbusDataset(args.data_path, split="validation", image_size=image_size)
