...
def main():
    ...
    model = DeepLabV3(num_classes=1, backbone=args.backbone, pretrained=args.pretrained).to(device)
    model_name = f"DeepLabV3-{args.backbone}"
    ...
    start_epoch, best = load_checkpoint(checkpoint_path, model, None, device)
    best_epoch = 1
    old_lr = args.lr
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        with torch.no_grad():
            for x, y in val_loader:
                ...
                loss = criterion(model(x), y)
                val_loss += loss.item()
                metrics_tracker.update(model(x), y, threshold=0.5)
