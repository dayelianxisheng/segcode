...
def parse_args():
    p = argparse.ArgumentParser(description="DeepLabV3 Inference")
...
def create_comprehensive_visualization(original_img, prob_map, binary_mask, overlay_img, threshold=0.5):
    ...
    fig.suptitle('DeepLabV3 Segmentation Results', fontsize=16, fontweight='bold')
...
def main():
    ...
    model = DeepLabV3(num_classes=1, backbone=args.backbone, pretrained=False).to(device)
