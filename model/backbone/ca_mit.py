...
from model.module.ca import ChannelAggregationFFN
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, qkv_bias=False, qk_scale=0., drop=0., attn_drop=0., proj_drop=0.,
                 act_layer="GELU", norm_layer=nn.LayerNorm, sr_ratio=1, drop_path=0.):
        super().__init__()
        ...
        self.caffn = ChannelAggregationFFN(embed_dims=dim,act_type=act_layer)
    ...
    def forward(self, x, H, W):
        ...
        x = x + self.drop_path(self.caffn(self.norm2(x), H, W))
        return x
...
class MixVisionTransformer(nn.Module):
    def __init__(self, in_channels=3, num_classes=1000, embed_dims=[32, 64, 160, 256],
        ...
        self.block2 = nn.ModuleList(
            [
                Block(
                    dim=embed_dims[1], num_heads=num_heads[1], qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
        ...
        self.block3 = nn.ModuleList(
            [
                Block(
                    dim=embed_dims[2], num_heads=num_heads[2], qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
        ...
        self.block4 = nn.ModuleList(
            [
                Block(
                    dim=embed_dims[3], num_heads=num_heads[3], qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
