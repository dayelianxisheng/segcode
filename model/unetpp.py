...
class UnetPP(nn.Module):
    def __init__(self, in_ch, out_ch, deepsupervision=False):
...
if __name__ == "__main__":
    # 1. 训练模式：开启深度监督
    model_train = UnetPP(in_ch=3, out_ch=1, deepsupervision=True)
    x_train = torch.randn(2, 3, 256, 256)  # batch=2，3通道，256×256图像
    outputs_train = model_train(x_train)
    print("训练模式输出数量：", len(outputs_train))
    print("训练模式单个输出形状：", outputs_train[0].shape)  # torch.Size([2, 1, 256, 256])

    # 2. 推理模式：关闭深度监督（默认）
    model_infer = UnetPP(in_ch=3, out_ch=1)  # deepsupervision默认False
    x_infer = torch.randn(1, 3, 256, 256)  # batch=1
    output_infer = model_infer(x_infer)
    print("推理模式输出形状：", output_infer.shape)  # torch.Size([1, 1, 256, 256])